...
async def add_user_to_chat(
    chat_user: ChatUserCreate, session: AsyncSession = Depends(get_session)
):
    """Добавление пользователя в чат"""
    chat_user = {"chat_id": chat_user.chat_id, "user_id": chat_user.user_id}
    ...
    if user:
        if chat:
            try:
                ...
                await session.rollback()
                return JSONResponse(
                    content={"message": "Не удалось добавить пользователя в чат"},
                    status_code=500,
                )
        else:
            return JSONResponse(
                content={"message": f"Не найден чат с id {chat_user['chat_id']}"}
            )
    else:
        return JSONResponse(
            content={"message": f"Не найден пользователь с id {chat_user['user_id']}"}
        )
