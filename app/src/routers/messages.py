...
async def post_message_func(
    message: CreateMessageSchema, current_user: dict = Depends(get_current_user)
):
    """Отправка сообщения"""
    ...
    if not chat:
        raise HTTPException(
            status_code=404, detail=f"Не обнаружен чат с id {message.chat_id}"
        )

    if not chat_member:
        raise HTTPException(
            status_code=403,
            detail=f"Пользователь с id {current_user.id} не является участником чата с id {message.chat_id}",
        )
...
async def get_messages_func(
    ...
    if not chat_member:
        raise HTTPException(
            status_code=403,
            detail=f"Пользователь с id {current_user.id} не является участником чата с id {chat_id}",
        )
...
async def edit_message_func(
    message_id: int,
    edited_message: EditMessageSchema,
    current_user: dict = Depends(get_current_user),
):
    ...
    if not chat_member:
        raise HTTPException(
            status_code=403, detail="Вы не являетесь участником чата данного сообщения"
        )

    if message.sender_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Вы не можете редактировать чужие сообщения"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Не удалось отредактировать сообщение"
        )
...
async def delete_message_func(
    ...
    if (message.sender_id != current_user.id) and (chat_member.is_admin == False):
        raise HTTPException(
            status_code=403,
            detail="Вы не можете удалять чужие сообщения или вы не являетесь админом чата",
        )
